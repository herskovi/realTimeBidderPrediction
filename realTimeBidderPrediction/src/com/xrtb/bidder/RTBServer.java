...
public class RTBServer implements Runnable {
	...
	public static final String SIMULATOR_ROOT = "simulator/exchange.html";
...
class Handler extends AbstractHandler {
	...
	public void handle(String target, Request baseRequest,
		...
		try {
			...
			{
				String page= ByteBuffer.wrap(
						(FileUtils.readAllBytes(FileUtils.getInputStreamFromPath((RTBServer.SIMULATOR_ROOT))))).toString();
			
				//String page = Charset.defaultCharset().decode(str);
			...
			if (target.contains(RTBServer.CAMPAIGN_URL)) {
				String page = Charset
						.defaultCharset()
						.decode(ByteBuffer.wrap(FileUtils.readAllBytes((FileUtils.getInputStreamFromPath(RTBServer.CAMPAIGN_ROOT))))).toString();
			...
			if (target.contains(RTBServer.LOGIN_URL)) 
			{
				String page = Charset
						.defaultCharset()
						.decode(ByteBuffer.wrap(FileUtils.readAllBytes(FileUtils.getInputStreamFromPath(RTBServer.LOGIN_ROOT)))).toString();
...
//								.get(RTBServer.LOGIN_ROOT)))).toString();
		...
		try {
			...
			String page = Charset
					.defaultCharset()
					.decode(ByteBuffer.wrap(FileUtils.readAllBytes(FileUtils.getInputStreamFromPath(target)))).toString();
